...
if (isset($this->session->userdata['logged_in'])) {
    ...
    header("location: ".base_url()."autentikasi/ldapLogin/logout");
}
...
function content_table($posts, $data_jadwal, $offset, $hak_akses, $username){
    ...
    foreach ($posts as $rows) {
        ...
        <tr>
            ...
            <!-- menuh2in tempat tapi kalo di delete jadi error ???... padahal cuma comment doang ???...
            <td>
                <table>'; 
                foreach ($schedule as $k_start_date => $v_start_date) {
                    foreach ($v_start_date as $k_end_date => $v_end_date) {
            ...
            <td class="containerxx" style="font-size:16px; vertical-align:middle;">
                '.status_persetujuan($rows['status'], $rows['alasan']).'
                <!--<ul class="progressbar">'.$status[$rows['status']].'</ul>-->
            </td>
            
            <td id="crud_view_'.$rows['nomor'].'" style="text-align:top; width:117px;">';
                if($hak_akses > 0){
                    echo'
                    <span>
                ...
                elseif($hak_akses == 0 && $rows['username'] == $username && $rows['flag_cetak'] == 0) {
                  echo '
                  <span>
                    <i id='.$rows['nomor'].' data-toggle="modal" data-target=".form-booking-edit" class="edit-kegiatan fa fa-edit fa-align-center" style="color:#00a65a; font-size:20px; cursor:pointer; width:20px"></i>
                  </span>&nbsp;&nbsp;|&nbsp;&nbsp;
                ...
                } elseif($hak_akses == 0 && $rows['username'] == $username && $rows['flag_cetak'] == 1) {
                  echo '
					<i id='.$rows['nomor'].' class="testing '.$rows['username'].' fa fa-file-pdf-o fa-align-center fa-lg faa-vertical" style="color:red; cursor:pointer; width:20px"></i>
                  <!--<span>
                    <i id='.$rows['nomor'].' class="view_final '.$rows['username'].' fa fa-file-pdf-o fa-align-center fa-lg faa-vertical" style="color:red; cursor:pointer; width:20px"></i>
                  </span>&nbsp;&nbsp;|&nbsp;&nbsp;-->
                  <span>
                    <i id='.$rows['nomor'].' rel="'.$rows['event_name'].'" class="del_kegiatan '.$rows['username'].' fa fa-trash fa-align-center fa-lg" style="color:red; cursor:pointer; width:20px"></i>
                  </span>';
                }
                echo '
            </td>';
                  ...
                  <tfoot>
                    <tr style="color:#fff; ">
                      ...
                      <th>Status Persetujuan</th>';
                        ...
                        if($hak_akses > 0){
                            echo'<th width="120px">Edit|View|Del</th>';
                        } else {
                            echo '<th width="260px"> width="260px" View|Del</th>';
                        }
                     echo '
                        <th>file<br>tor</th>
